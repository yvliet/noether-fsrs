...
export const FlashcardsView: React.FC = React.memo(() => {
  ...
  useEffect(() => {
    ...
    window.addEventListener('noether:fsrs-updated', handleFsrsUpdated);
    return () => {
      isMounted = false;
      window.removeEventListener('noether:fsrs-updated', handleFsrsUpdated);
    };
  ...
  return (
    <div className="flex flex-col h-full select-none text-xs">
      {/* Top Header */}
      <div className="h-9 px-3 flex items-center justify-between text-[var(--noether-text-muted)] shrink-0 border-b border-[var(--noether-border-base)]">
        <div className="flex items-center gap-1.5 font-medium text-xs text-[var(--noether-text-secondary)]">
          <Brain02Icon size={14} className="text-[var(--noether-accent)]" />
        ...
        <div className="p-2.5 pb-1.5">
          ...
          >
            <div className="flex items-center gap-2">
              ...
              <div>
                <div className="text-[11px] font-medium text-[var(--noether-text-primary)]">
                  Review Flashcards
                </div>
                <div className="text-[10px] text-[var(--noether-text-muted)]">
                  {dueCardCount > 0 ? `${dueCardCount} cards due` : `${cardsInNote.length} cards in note`}
                </div>
              </div>
            </div>

            <kbd className="px-1.5 py-0.5 rounded bg-[var(--noether-bg-input)] text-[9px] font-mono text-[var(--noether-text-muted)] border border-[var(--noether-border-base)]">
              Ctrl+Shift+R
            </kbd>
          </button>
        </div>
      ...
      {isSearchOpen && (
        <div className="px-2.5 py-1.5">
          <div className="flex items-center gap-2 px-2.5 py-1.5 rounded-md bg-[var(--noether-bg-input)] text-xs text-[var(--noether-text-primary)] border border-[var(--noether-border-base)]">
      ...
      {/* Filter Bar */}
      {cardsInNote.length > 0 && (
        <div className="flex items-center gap-1.5 px-3 py-2 mb-1 flex-wrap border-b border-[var(--noether-border-base)]">
          {[
      ...
      <div className="flex-1 overflow-y-auto px-2.5 py-1.5 custom-scrollbar flex flex-col gap-2">
        {filteredCards.length === 0 ? (
          <div className="text-center py-12 flex flex-col items-center justify-center gap-2 select-none text-[var(--noether-text-muted)]">
              ...
              >
                <div className="flex items-center justify-between text-[10px]">
                  <span className="font-semibold uppercase tracking-wider text-[var(--noether-text-muted)] flex items-center gap-1">
                    <span className={`w-1.5 h-1.5 rounded-full ${isFlipped ? 'bg-[var(--noether-accent)]' : 'bg-emerald-400'}`} />
                    {card.card_type.replace(/_/g, ' ')}
                  </span>

                  <span className="text-[10px] text-[var(--noether-text-faint)] flex items-center gap-1 group-hover:text-[var(--noether-text-muted)]">
                ...
                {isFlipped ? (
                  <div className="text-[12px] text-[var(--noether-text-primary)] font-normal leading-snug">
                    {isCloze ? (
                      card.front_text.split(/(\{[^{}]+\}|==[^=\n]+==)/g).map((part, i) =>
                        (part.startsWith('{') && part.endsWith('}')) || (part.startsWith('==') && part.endsWith('==')) ? (
                          <span
                            key={i}
                            className="inline-block px-1.5 py-0.5 mx-0.5 rounded-[5px] bg-[#222222] border border-[#383838] shadow-[0_1px_2px_rgba(0,0,0,0.35)] text-[var(--noether-accent)] font-mono text-[10px]"
                          >
                            [...]
                          </span>
                        ) : (
                  ...
                  <div className="text-[11px] text-[var(--noether-text-secondary)] leading-relaxed bg-[var(--noether-bg-input)] p-2 rounded border border-[var(--noether-border-base)]">
                    <div className="text-[9px] uppercase tracking-wider text-[var(--noether-accent)] font-semibold mb-0.5">
                      Answer
                    </div>
                    {card.back_text}
                  </div>
